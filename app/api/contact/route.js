...
export async function POST(request) {
  try {
    ...
    if (clientIp && clientIp !== 'Localhost (Development)') {
      ...
      systemLocation = 'Localhost (Development)';
    }
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <title>Focus Support Inquiry</title>
        <style>
          body {
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <table class="field-table">
              ...
              <tr>
                ...
                <td class="field-value highlight">${subject}</td>
              </tr>
              <tr>
                <td class="field-label" style="padding-top: 20px;">Message Detail</td>
          ...
          <div class="footer">
            Focus App Support &bull; Secure Local Database System
          </div>
        </div>
      </body>
      </html>
    ...
    const mailOptions = {
      from: `"Focus App Support" <${process.env.MAIL_USER || 'support@getfocus.online'}>`,
      to: process.env.RECIPIENT_EMAIL || 'ashazpathan8@gmail.com',
      subject: `✦ Support Inquiry: ${subject} (from ${fullName})`,
      text: `Focus App Support Inquiry Received!\n\nName: ${fullName}\nEmail: ${email}\nSubject: ${subject}\n\nMessage:\n${message}\n\nSystem Metadata:\nIP: ${clientIp}\nResolved Location: ${systemLocation}\nTimestamp: ${timestamp}`,
      html: htmlContent
    };

    // Trigger SMTP Email delivery
    await transporter.sendMail(mailOptions);
    console.log(`[SMTP Success] Support inquiry email successfully sent for: ${fullName} (${email})`);
    ...
    console.error('Nodemailer SMTP Sending error in Next.js contact route:', error);
    return NextResponse.json({
      success: false,
      error: error.message || 'Failed to process support request. Please try again later.'
    }, { status: 500 });
  }
}
